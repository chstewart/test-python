...
class BallClock:
  ...
  def add_ball(self):
    ...
    if len(self.minute_track) == 5:
      self.main_track = self.main_track + list(reversed(self.minute_track[:-1]))
      self.minute_track = [self.minute_track.pop()]
      self.add_five_minute_ball()

  def add_five_minute_ball(self):
    """
    ...
    """
    ball: int = self.minute_track.pop()
    self.five_minute_track.append(ball)

    if len(self.five_minute_track) == 12:
      self.main_track = self.main_track + list(reversed(self.five_minute_track[:-1]))
      self.five_minute_track = [self.five_minute_track.pop()]
      self.add_hour_ball()

  def add_hour_ball(self):
    """
    ...
    """
    ball: int = self.five_minute_track.pop()
    self.hour_track.append(ball)

    if len(self.hour_track) == 12:
      self.main_track = self.main_track + list(reversed(self.hour_track[:-1]))
      self.main_track = self.main_track + [self.hour_track.pop()]
      self.hour_track = []
      self.days += 0.5
